...
in
{
  ...
  programs.caelestia = lib.mkIf (isLinux && isDesktop) (
    let
  ...
  xdg.configFile."caelestia/shell.json".force = lib.mkIf (isLinux && isDesktop) true;

  # Stub so Hyprland can source it before caelestia writes the real file
  xdg.configFile."caelestia/hypr-user.conf" = lib.mkIf (isLinux && isDesktop) {
